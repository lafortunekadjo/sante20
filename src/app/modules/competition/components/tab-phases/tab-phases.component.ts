...
import { ModalTirageFinaleComponent } from '../modal-tirage-finale/modal-tirage-finale.component';
...
export class TabPhasesComponent implements OnInit {
  ...
  bracketEditMode = signal(false); // ← signal correct pour le mode bracket
  planifMasseOpen = signal(false);
  tirageFinaleOpen = signal(false);
  savingPlanif    = signal(false);
  tousStades      = signal<StadeDTO[]>([]);

  private classementMap = new Map<number, any[]>();
  ...
  ngOnInit(): void {
    this.api.getPhases(this.competition.id).subscribe(phases => {
      ...
      // Priorité à la phase GROUPE EN_COURS
      const phaseGroupe = phases.find(p =>
        p.type === TypePhase.GROUPE && p.statut === 'EN_COURS');
      const premiere = phaseGroupe ?? phases[0];
      if (premiere) this.selectPhase(premiere);
    });
  ...
  selectPhase(phase: PhaseDTO): void {
    ...
    if (phase.type === TypePhase.GROUPE) {
            ...
            this.api.getClassement(this.competition.id, phase.id, p.id)
              .subscribe(cl => {
                ...
                this.poules.update(list => list.map(poule =>
                  poule.id === p.id ? { ...poule, classement: cl } : poule
                ));
              });
          });
      ...
      forkJoin([
        ...
        this.matchApi.getMatchsByPhase(this.competition.id, phase.id)
  ...
  ouvrirMatchEdit(match: MatchDTO): void {
    ...
    // FIX : utiliser bracketEditMode signal, pas isBracketEdit()
    this.bracketEditMode.set(this.activePhase()?.type === 'ELIMINATOIRE');
  ...
  fermerMatchEdit(): void {
    ...
    setTimeout(() => {
      this.matchSelectionne.set(null);
      this.bracketEditMode.set(false);
    }, 300);
  }
  ...
  peutPasserPhaseActuelle(): boolean {
    ...
    const phaseGroupe = this.phases().find(p => p.type === TypePhase.GROUPE);
    if (!phaseGroupe || phaseGroupe.statut !== 'EN_COURS') return false;

    const phaseFinale = this.phases().find(p => p.type === TypePhase.ELIMINATOIRE);
    if (phaseFinale?.statut === 'EN_COURS') return false;

    const journees = [...this.journeesMap.values()].flat() as any[];
    if (!journees.length) return false;
    return journees.every((j: any) =>
      j.statut === 'TERMINEE' || j.statut === 'TERMINE');
  }

  canPasserPhase(): boolean {
    return this.peutPasserPhaseActuelle();
  }

  passerPhaseSuivante(): void {
    this.tirageFinaleOpen.set(true);
  }

  onTirageFinaleValidated(): void {
    this.tirageFinaleOpen.set(false);
    this.api.getPhases(this.competition.id).subscribe(phases => {
      this.phases.set(phases);
