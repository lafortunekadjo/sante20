...
export class AuthService {
  ...
  private initializeAuthState(): void {
    ...
    if (storedToken && !this.jwtHelper.isTokenExpired(storedToken)) {
        ...
        // 🚨 CORRECTION CLÉ : Utiliser setTimeout pour décaler l'appel HTTP 
        // et casser la dépendance circulaire synchrone.
        setTimeout(() => {
        ...
        }, 0); // Décalage minimal (0ms) pour casser la chaîne synchrone
    } else {
  ...
  private fetchUserInfoFromToken(): Observable<any> {
    ...
    return this.http.get<any>(this.userInfoUrl, {
      ...
      tap(userInfo => {
          ...
          this.roles = userInfo.roles || [];
          this.username = userInfo.username || null;
          this.groupeId = userInfo.groupe.id || null;
  ...
  */
  ...
  updateUserProfile(userData: User): Observable<User> {
    ...
    getCurrentGroup(): Observable<Groupe> {
    // 💡 Correction: l'URL pour getCurrentGroup doit être la bonne (apiGroupesConnect),
    // pas apiUrl qui est pour le login. J'ai utilisé apiGroupesConnect.
    return this.http.get<Groupe>(this.apiGroupesConnect).pipe(
